	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				//Posterize and open
#if _USEPOS_ON
				Main = saturate( floor((Main) * _Posterize) / (_Posterize - 1));
#endif
				fixed pro = (min((1.0 - max(max(i.vertexColor.r, i.vertexColor.g), i.vertexColor.b)), i.vertexColor.a));
				//Main = fixed4(Main.rgb  ,  pro) * i.vertexColor * i.vertexColor.a *  Main.a;
#ifdef Enable_Bloom
				Main = fixed4(Main.rgb, pro) * i.vertexColor * i.vertexColor.a *  Main.a  * exp(_EmissionGain * 5.0f);
#else
				Main = fixed4(Main.rgb, pro) * i.vertexColor * i.vertexColor.a *  Main.a;
#endif
				//Masktexture
				...
				UNITY_APPLY_FOG_COLOR(i.fogCoord, Main, unity_FogColor*Main.a);
				return Main;
			}
			ENDCG
		}
